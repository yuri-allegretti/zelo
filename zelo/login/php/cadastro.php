...
$nome            = isset($dados['nome'])            ? trim($dados['nome'])            : '';//ap
...
// Validações
if (empty($nome) || empty($sobrenome) || empty($email) || empty($senha) || empty($data_nasc)) {//ap
...
$stmt = $pdo->prepare('INSERT INTO cadastro (nome, sobrenome, email, senha, data_nasc) VALUES (?, ?, ?, ?, ?)');//ap ?
$stmt->execute([$nome, $sobrenome, $email, $hash, $data_nasc]);//ap
